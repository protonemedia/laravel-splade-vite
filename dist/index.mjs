...
function LaravelSplade(config = {}) {
  ...
  const run = async (command) => {
    ...
    if (verbose) {
      console.log(stdout);
      console.log(
        failed ? `Laravel Splade Vite plugin: Components failed to process, exit code: ${exitCode}` : "Laravel Splade Vite plugin: Components processed"
      );
    }
  ...
  return {
    ...
    async buildStart() {
      ...
      await generatePluginManifest();
      const extractComponentsCommand = resolvedConfig.isProduction ? [artisan, "splade:core:build-components", "--unprocessed"] : [artisan, "splade:core:clear-components"];
      if (verbose) {
